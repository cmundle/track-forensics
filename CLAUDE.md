...
Eight tracks under `calibration/v5/`, JSON only (`.gitignore` excludes the stems; no source material is ever committed). One row per failure mode: house at a fixed tempo, swung hip-hop, live band with a floating tempo, ambient with no pulse, drum & bass at 170, plus a funk record and two short clips. **The ambient row is the important one and the easiest to skip** — it exists so that "the tool correctly refused" is a tested outcome. A tool only ever tested on material it handles well will quietly learn to always answer.
