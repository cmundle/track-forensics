...
The cost is real and worth stating plainly: **`centroid_mean`, `rolloff_mean` and `rhythm.bpm` survive as fields that look authoritative and are not.** Nothing in the type system distinguishes them from the corrected ones. Docstrings in `schemas.py` are the only guard, and docstrings are easy to ignore — `heuristics.py` had four thresholds tuned against a `centroid_mean` contaminated by silent frames, and one of them (`dark_centroid_hz`) made its label unreachable on every real bass stem ever measured. W8B migrated three of the four and `sustained sub` now fires on the ambient row's bass stem, which is the first time it has fired on real material — but the migration was not visible in `calibration/v5/` until the baseline was regenerated long afterwards, so treat a label's absence there as evidence only if the baseline is known to be current. Revisit at v6: `rolloff_mean` now has **no reader anywhere** outside the backends that produce it, `centroid_mean` is down to two gates W8B measured and deliberately left, and `rhythm.bpm` is not removable at all — it is the coarse input `refine_bpm` starts from.
...
Ten tracks under `calibration/v5/`, JSON only (`.gitignore` excludes the stems; no source material is ever committed). One row per failure mode: house at a fixed tempo, swung hip-hop, live band with a floating tempo, ambient with no pulse, drum & bass at 170, a funk record, plus four added afterwards to take a bug from one failing example to two — a second reverberant kit, a working drum & bass guard, a genuine shuffle, and a second fixed-tempo guard. Every row is a full-length record; two short clips were carried through the v4 → v5 cycle and dropped afterwards, since Demucs put nearly all of both into `other` and they were too short for tempo drift to show. **The ambient row is the important one and the easiest to skip** — it exists so that "the tool correctly refused" is a tested outcome. A tool only ever tested on material it handles well will quietly learn to always answer.

**Thresholds measured before the clips were dropped cite "W8B's eight-track corpus" and "40 stems" (8 tracks × 5 sources).** That is the corpus as it stood during the v4 → v5 cycle, and those numbers are left as measured — a measurement is a record of what was run, not a claim about what `calibration/v5/` holds today. Re-measuring anything now runs on six tracks and 30 stems and will not reproduce them. Say which corpus a new number came from.
